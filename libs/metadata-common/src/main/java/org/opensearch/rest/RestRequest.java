...
public class RestRequest implements ToXContent.Params {
     ...
     * with an unpooled copy. This is supposed to be used before passing requests to {RestHandler} instances that can not safely
     * handle http requests that use pooled buffers as determined by {RestHandler#allowsUnsafeBuffers()}.
     */
    ...
    public BytesReference originalContent() {
        Optional<BytesReference> bytesReference = this.httpRequest.originalContent();
        if (bytesReference.isPresent()) {
            logger.debug("Resolved original content from request of {}", this.httpRequest.getClass());
        } else {
            logger.debug("Original content not present on {}, falling back to content", this.httpRequest.getClass());
        }
        return bytesReference.orElseGet(this::content);
    }
     ...
     * {RestHandler} treats the {@code source} parameter like the body content.
     */
    ...
    public static class BadParameterException extends RuntimeException {
        ...
        public BadParameterException(final IllegalArgumentException cause) {
            super(cause);
        }
