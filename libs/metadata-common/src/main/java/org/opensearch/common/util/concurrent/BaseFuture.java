...
public abstract class BaseFuture<V> implements Future<V> {
    ...
    */
    ...
    protected boolean blockingAllowed() {
        return assertNotTransportThread(BLOCKING_OP_REASON)
            && assertNotScheduleThread(BLOCKING_OP_REASON)
            && assertNotClusterStateUpdateThread(BLOCKING_OP_REASON)
            && assertNotClusterManagerUpdateThread(BLOCKING_OP_REASON);
    }
    ...
    public static boolean assertNotClusterStateUpdateThread(String reason) {
        assert Thread.currentThread().getName().contains("clusterApplierService#updateTask") == false : "Expected current thread ["
            + Thread.currentThread()
    ...
    public static boolean isTransportThread(Thread t) {
        ...
        for (String s : Arrays.asList("http_server_worker", "transport_worker", "__mock_network_thread")) {
    ...
    private static boolean isClusterManagerUpdateThread() {
        return Thread.currentThread().getName().contains("clusterManagerService#updateTask")
            || Thread.currentThread().getName().contains("masterService#updateTask");
    }
